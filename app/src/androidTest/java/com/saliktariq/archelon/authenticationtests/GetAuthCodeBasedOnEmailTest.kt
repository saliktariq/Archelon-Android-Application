...
class GetAuthCodeBasedOnEmailTest {
    ...
    fun getPassword() {
        val newUserData =
            Authentication(1, "saliktariq", "password".hashCode().toLong(), "mtariq01@dcs.bbk.ac.uk", "Salik", 7500)
        GlobalScope.launch {
            authenticationDao.insertAuthenticationData(newUserData)
            val authCode = authenticationDao.getAuthCodeBasedOnEmail("mtariq01@dcs.bbk.ac.uk")

            assertEquals(authCode, 7500)
        }
