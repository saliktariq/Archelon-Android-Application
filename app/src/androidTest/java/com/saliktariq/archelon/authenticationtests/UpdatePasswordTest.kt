...
class UpdatePasswordTest{
    ...
    fun updatePassword() {
        ...
        val newUserData = Authentication(1,"saliktariq",10001000L,"mtariq01@dcs.bbk.ac.uk","Salik",7500)
        GlobalScope.launch {
            authenticationDao.insertAuthenticationData(newUserData)
            val updatedPassword = "newpassword"
            authenticationDao.updatePassword(updatedPassword.hashCode().toLong(),"mtariq01@dcs.bbk.ac.uk")
            val userData = authenticationDao.getUserData("saliktariq")
            assertEquals("newpassword".hashCode().toLong(),userData?.pwd)
        }
